import L from 'leaflet';
import { showFormattedDate } from '../../utils';

class HomeView {
    constructor() {
        this._map = null;
        this._markers = [];
        this._tileLayers = {};
        this._stories = [];
    }

    displayStories(stories) {
        this._stories = stories;
        this._renderStoryList(stories);
    }

    _renderStoryList(stories) {
        const storyListElement = document.getElementById('story-list');
        if (!storyListElement) return;

        storyListElement.innerHTML = '';

        if (stories.length === 0) {
            storyListElement.innerHTML = '<p>No stories found matching your search.</p>';
            return;
        }

        stories.forEach((story, index) => {
            const storyItem = document.createElement('article');
            storyItem.classList.add('story-item');
            storyItem.dataset.storyId = story.id;
            storyItem.dataset.index = index;

            storyItem.innerHTML = `
        <img src="${story.photoUrl}" alt="Photo of ${story.name}'s story: ${story.description.substring(0, 50)}">
        <div class="story-item-content">
          <h3>${story.name}</h3>
          <p>${story.description}</p>
          <p class="story-date"><small>${showFormattedDate(story.createdAt)}</small></p>
        </div>
      `;

            storyItem.addEventListener('click', () => {
                window.location.hash = `#/story/${story.id}`;
            });

            storyItem.addEventListener('mouseenter', () => {
                this._highlightMarker(index);
            });

            storyItem.addEventListener('mouseleave', () => {
                this._unhighlightMarker(index);
            });

            storyItem.addEventListener('focus', () => {
                this._highlightMarker(index);
            });

            storyItem.addEventListener('blur', () => {
                this._unhighlightMarker(index);
            });

            storyItem.setAttribute('tabindex', '0');
            storyItem.setAttribute('role', 'button');
            storyItem.setAttribute('aria-label', `View story by ${story.name}: ${story.description.substring(0, 50)}`);

            storyItem.addEventListener('keydown', (e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    window.location.hash = `#/story/${story.id}`;
                }
            });

            storyListElement.appendChild(storyItem);
        });
    }

    initializeMap(stories) {
        const mapElement = document.getElementById('map');
        if (!mapElement) return;

        this._map = L.map('map').setView([-6.2088, 106.8456], 5);

        this._tileLayers = {
            street: L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
            }),
            satellite: L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
                attribution: 'Tiles &copy; Esri'
            }),
            topo: L.tileLayer('https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png', {
                attribution: 'Map data: &copy; OpenStreetMap contributors, SRTM | Map style: &copy; OpenTopoMap'
            })
        };

        this._tileLayers.street.addTo(this._map);

        L.control.layers({
            "Street Map": this._tileLayers.street,
            "Satellite": this._tileLayers.satellite,
            "Topographic": this._tileLayers.topo
        }).addTo(this._map);

        this._addMarkersToMap(stories);
    }

    _addMarkersToMap(stories) {
        stories.forEach((story, index) => {
            if (story.lat && story.lon) {
                const marker = L.marker([story.lat, story.lon]).addTo(this._map)
                    .bindPopup(`<b>${story.name}</b><br>${story.description}<br><small>${showFormattedDate(story.createdAt)}</small>`);

                this._markers[index] = marker;

                marker.on('click', () => {
                    const storyItem = document.querySelector(`[data-index="${index}"]`);
                    if (storyItem) {
                        storyItem.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        storyItem.classList.add('highlighted');
                        storyItem.focus();
                        setTimeout(() => storyItem.classList.remove('highlighted'), 2000);
                    }
                });
            }
        });
    }

    _highlightMarker(index) {
        const marker = this._markers[index];
        if (marker) {
            marker.openPopup();
        }
    }

    _unhighlightMarker(index) {
        const marker = this._markers[index];
        if (marker) {
            marker.closePopup();
        }
    }

    initializeFilter(stories) {
        const filterInput = document.getElementById('location-filter');
        if (!filterInput) return;

        filterInput.addEventListener('input', (e) => {
            const searchTerm = e.target.value.toLowerCase();
            const filteredStories = stories.filter(story =>
                story.name.toLowerCase().includes(searchTerm) ||
                story.description.toLowerCase().includes(searchTerm)
            );

            this._renderStoryList(filteredStories);
            this._updateMapMarkers(filteredStories);
        });

        filterInput.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                filterInput.value = '';
                filterInput.dispatchEvent(new Event('input'));
            }
        });

        filterInput.setAttribute('aria-label', 'Filter stories by name or description. Press Escape to clear.');
    }

    _updateMapMarkers(filteredStories) {
        this._markers.forEach(marker => {
            if (marker) this._map.removeLayer(marker);
        });
        this._markers = [];
        this._addMarkersToMap(filteredStories);
    }

    showError(message) {
        const storyListElement = document.getElementById('story-list');
        if (storyListElement) {
            storyListElement.innerHTML = `<p role="alert">Failed to load stories. Please try again later. Message: ${message}</p>`;
        }

        const mapElement = document.getElementById('map');
        if (mapElement) {
            mapElement.innerHTML = '<p role="alert">Map cannot be displayed because stories could not be loaded.</p>';
        }
    }
}

export default HomeView;